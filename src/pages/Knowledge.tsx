  ...
  return (
    <div className="min-h-screen bg-flow-background text-flow-foreground flex flex-col bg-[#0c0e16] bg-[radial-gradient(circle_at_center,rgba(30,41,59,0.4)_0,rgba(12,14,22,0.8)_50%)]">
      ...
      <main className="flex-1 container mx-auto px-4 py-16">
        <TransitionWrapper>
          ...
          >
            ...
            <div className="flex flex-col md:flex-row gap-4">
              ...
              <div className="flex gap-2">
                ...
                <TooltipProvider>
                  <Tooltip>
                    <TooltipTrigger asChild>
                      <Button
                        variant="ghost"
                        className="border-flow-border hover:bg-flow-muted/20 transition-all duration-300 h-11"
                ...
                <div className="flex border border-flow-border rounded-md overflow-hidden">
                  <TooltipProvider>
                    <Tooltip>
                      <TooltipTrigger asChild>
                        <Button
                          variant="ghost"
                          className={`h-11 px-3 rounded-none ${viewMode === 'grid' ? 'bg-flow-accent/20' : 'hover:bg-flow-muted/20'}`}
                  ...
                  <TooltipProvider>
                    <Tooltip>
                      <TooltipTrigger asChild>
                        <Button
                          variant="ghost"
                          className={`h-11 px-3 rounded-none ${viewMode === 'list' ? 'bg-flow-accent/20' : 'hover:bg-flow-muted/20'}`}
          ...
          <Tabs defaultValue="all" value={activeTab} onValueChange={setActiveTab} className="mb-8">
            <TabsList className="bg-flow-muted/20 border border-flow-border p-1 mb-6">
              <TabsTrigger value="all" className="data-[state=active]:bg-flow-accent/20">All</TabsTrigger>
              <TabsTrigger value="internal" className="data-[state=active]:bg-flow-accent/20">Internal</TabsTrigger>
              <TabsTrigger value="client-facing" className="data-[state=active]:bg-flow-accent/20">Client-facing</TabsTrigger>
            </TabsList>
            ...
            <TabsContent value="all" className="mt-0">
              ...
              {filteredItems.length > 0 ? (
                <div className={viewMode === 'grid' ? 'grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6' : 'space-y-4'}>
                  {filteredItems.map((item) => (
                    viewMode === 'grid' ? (
                      <Card 
                        key={item.id}
                        className="overflow-hidden border-flow-border/40 bg-flow-background/40 backdrop-blur-md hover:border-flow-accent/40 transition-all duration-300 cursor-pointer"
                        onClick={() => handleViewItem(item)}
                      ...
                      >
                        <CardHeader className="p-5 pb-0">
                          <div className="flex items-center justify-between mb-2">
                            <div 
                              className="flex items-center justify-center px-3 py-1 rounded-full text-xs backdrop-blur-sm" 
                              style={{ backgroundColor: `${getTypeColor(item.type)}40` }}
                            >
                              {getTypeIcon(item.type)}
                              <span className="ml-1.5 capitalize">{item.type}</span>
                            </div>
                            <Badge variant="outline" className={item.category === 'internal' ? 'bg-blue-500/10 text-blue-400 border-blue-500/30' : 'bg-purple-500/10 text-purple-400 border-purple-500/30'}>
                              {item.category}
                            </Badge>
                          </div>
                          <CardTitle className="text-lg">{item.title}</CardTitle>
                          <CardDescription className="text-sm text-flow-foreground/70">
                            {item.author}
                          </CardDescription>
                        </CardHeader>
                        <CardContent className="p-5 pt-3">
                          <p className="text-sm line-clamp-3">{item.preview}</p>
                        </CardContent>
                        <CardFooter className="p-5 pt-0 flex justify-between">
                          <div className="flex items-center space-x-3 text-xs text-flow-foreground/60">
                          ...
                          <div className="flex">
                            <TooltipProvider>
                              <Tooltip>
                                <TooltipTrigger asChild>
                                  <button 
                                    className="text-flow-foreground/60 hover:text-purple-500 transition-colors"
                                    onClick={(e) => {
                                      e.stopPropagation();
                                ...
                                <TooltipContent>
                                  {item.isBookmarked ? 'Remove bookmark' : 'Add to bookmarks'}
                                </TooltipContent>
                              </Tooltip>
                            </TooltipProvider>
                          </div>
                        ...
                        <div className="p-5 pt-0 pb-3">
                          <Progress value={item.completion} className="h-1 bg-flow-border/30" indicatorClassName="bg-gradient-to-r from-flow-accent to-purple-500" />
                          <div className="mt-1 flex justify-between items-center text-xs text-flow-foreground/60">
                      ...
                      <GlassMorphism
                        key={item.id}
                        intensity="low"
                        className="p-4 border-flow-border/40 hover:border-flow-accent/40 transition-all duration-300 cursor-pointer"
                        onClick={() => handleViewItem(item)}
                        onMouseEnter={() => setHoveredCard(item.id)}
                        onMouseLeave={() => setHoveredCard(null)}
                      >
                        <div className="flex">
                          <div className="flex-shrink-0 w-12 h-12 rounded-md flex items-center justify-center mr-4" style={{ backgroundColor: getTypeColor(item.type) }}>
                            {getTypeIcon(item.type)}
                          </div>
                          <div className="flex-1 min-w-0">
                            ...
                            <p className="text-sm line-clamp-2 mb-2 text-flow-foreground/80">{item.preview}</p>
                            <div className="flex items-center justify-between">
                              <div className="flex items-center space-x-4 text-xs text-flow-foreground/60">
                                <div className="flex items-center">
                                  <Eye className="h-3 w-3 mr-1" />
                                  {item.views}
                                </div>
                ...
                <div className="flex flex-col items-center justify-center p-10 text-center">
                  <FileText className="h-16 w-16 text-flow-foreground/20 mb-4" />
                  <h3 className="text-xl font-medium mb-2">No items found</h3>
                  <p className="text-flow-foreground/60 max-w-sm">
                    We couldn't find any knowledge items matching your current filters. Try adjusting your search criteria or reset filters.
                  </p>
                  <Button 
                    variant="outline"
                    className="mt-6"
            ...
            <TabsContent value="internal" className="mt-0">
              {/* This content will be rendered automatically by the effect that filters based on activeTab */}
      ...
      <Dialog open={isDialogOpen} onOpenChange={setIsDialogOpen}>
        <DialogContent className="max-w-3xl bg-flow-background border border-flow-border p-6 max-h-[90vh] overflow-y-auto">
          {selectedItem && (
            <>
              <DialogHeader>
                <div className="flex items-center gap-3 mb-2">
                  ...
                  <div>
                    ...
                    <DialogDescription className="text-flow-foreground/70">
                      {selectedItem.author} • {selectedItem.lastUpdated}
                    </DialogDescription>
                  </div>
                </div>
              </DialogHeader>
              ...
              <div className="mt-3">
                <div className="flex flex-wrap items-center gap-2 mb-6">
                  <Badge variant="outline" className="bg-flow-accent/10 text-flow-accent border-flow-accent/30">
                    {selectedItem.category}
                  </Badge>
                  {selectedItem.tags.map(tag => (
                    <Badge key={tag} variant="outline" className="bg-flow-muted/30 border-flow-border/50">
                      {tag}
                    </Badge>
                  ))}
                </div>
                
                <div className="text-lg mb-6">{selectedItem.preview}</div>
                ...
                <div className="flex items-center justify-between mb-6">
                  <div className="flex items-center gap-4">
                    <div className="flex items-center gap-1 text-sm">
                      <Eye className="h-4 w-4 text-flow-foreground/70" />
                      <span>{selectedItem.views} views</span>
                    </div>
                    <div className="flex items-center gap-1 text-sm">
                      <ThumbsUp className="h-4 w-4 text-flow-foreground/70" />
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    >
                      <Bookmark className={`h-3 w-3 mr-1 ${selectedItem.isBookmarked ? 'fill-purple-500 text-purple-500' : ''}`} />
                      {selectedItem.isBookmarked ? 'Bookmarked' : 'Bookmark'}
                    </Button>
                  </div>
                </div>
                ...
                <GlassMorphism className="p-4 mb-6">
                  <div className="flex items-center justify-between mb-2">
                    ...
                    <span className="text-sm text-flow-foreground/70">{selectedItem.completion}%</span>
                  </div>
                  <Progress value={selectedItem.completion} className="h-2" />
                </GlassMorphism>
                
                <div className="text-center text-sm text-flow-foreground/60">
                  This is a preview of the knowledge item. In a real application, this would display the full content.
                </div>
              </div>
            </>
          )}
